...
SELECT T.NOME AS TREINADOR, P.NOME AS POKEMON, PT.TIME_ID, PT.pokemon_id
...
SELECT T.NOME AS TREINADOR, P.NOME AS POKEMON, P.POkemon_id
FROM PROJETO_FINAL.TREINADOR T
JOIN PROJETO_FINAL.TREINADOR_TIME TT ON T.TREINADOR_ID = TT.TREINADOR_ID
JOIN PROJETO_FINAL.POKEMON_TIME PT ON TT.TIME_ID = PT.TIME_ID
JOIN PROJETO_FINAL.POKEMON P ON PT.POKEMON_ID = P.POKEMON_ID
...
SELECT P.NOME AS POKEMON, M.NOME AS MOVIMENTO, P.POKEMON_ID
...
SELECT T.NOME AS TREINADOR, B.GINASIO_ID, B.BATALHA_ID
FROM PROJETO_FINAL.TREINADOR T
...
WHERE B.GINASIO_ID = 8; 

-- Listar todos os treinadores que venceram batalhas em uma determinada região:
SELECT T.NOME AS TREINADOR, B.GINASIO_ID, B.DATA, B.BATALHA_ID
FROM PROJETO_FINAL.TREINADOR T
JOIN PROJETO_FINAL.TREINADOR_BATALHA TB ON T.TREINADOR_ID = TB.TREINADOR_ID
JOIN PROJETO_FINAL.BATALHA B ON TB.BATALHA_ID = B.BATALHA_ID
...
JOIN
    PROJETO_FINAL.TREINADOR_BOLSA TB ON T.TREINADOR_ID = TB.TREINADOR_ID
JOIN
    PROJETO_FINAL.BOLSA B ON TB.BOLSA_ID = B.BOLSA_ID
JOIN
    PROJETO_FINAL.TREINADOR T2 ON T.TREINADOR_ID = T2.TREINADOR_ID
JOIN
...
SELECT T.NOME AS TREINADOR, I.DESCRICAO, I.TIPO_ITEM
FROM PROJETO_FINAL.TREINADOR T
...
SELECT DISTINCT T.NOME AS TREINADOR, TP.NOME_TIPO
FROM PROJETO_FINAL.TREINADOR T
JOIN PROJETO_FINAL.TREINADOR_TIME TT ON T.TREINADOR_ID = TT.TREINADOR_ID
JOIN PROJETO_FINAL.POKEMON_TIME PT ON TT.TIME_ID = PT.TIME_ID
JOIN PROJETO_FINAL.POKEMON P ON PT.POKEMON_ID = P.POKEMON_ID
...
JOIN PROJETO_FINAL.POKEMON_TIPO_POKEMON PTP ON P.POKEMON_ID = PTP.POKEMON_ID
JOIN PROJETO_FINAL.TIPO_POKEMON TP ON PTP.TIPO_POKEMON_ID = TP.TIPO_POKEMON_ID
WHERE TP.NOME_TIPO = 'Normal' AND P.LVL > 90;

-- Listar todos os Pokémon que compartilham um movimento específico e são do mesmo tipo:
SELECT P1.NOME AS POKEMON_1, P2.NOME AS POKEMON_2, M.NOME AS MOVIMENTO
FROM PROJETO_FINAL.POKEMON_MOVE PM1
...
WHERE PTP1.TIPO_POKEMON_ID = PTP2.TIPO_POKEMON_ID
...
SELECT I.NOME AS ITEM, COUNT(DISTINCT B.BOLSA_ID) AS NUMERO_BOLSAS, 
       ARRAY_TO_STRING(ARRAY_AGG(T.NOME), ', ') AS TREINADORES_COM_BOLSA
FROM PROJETO_FINAL.ITEM I
...
JOIN
    PROJETO_FINAL.POKEMON_TIME PT ON TT.TIME_ID = PT.TIME_ID
JOIN
    PROJETO_FINAL.POKEMON_TIPO_POKEMON PTP ON PT.POKEMON_ID = PTP.POKEMON_ID
JOIN
...
FROM
    PROJETO_FINAL.TREINADOR T
JOIN
    PROJETO_FINAL.TREINADOR_TIME TM ON T.TREINADOR_ID = TM.TREINADOR_ID
JOIN
    PROJETO_FINAL.POKEMON_TIME PT ON TM.TIME_ID = PT.TIME_ID
JOIN
    PROJETO_FINAL.POKEMON P ON PT.POKEMON_ID = P.POKEMON_ID
JOIN
...
SELECT *
FROM PROJETO_FINAL.VIEW_ALL_POKEMONS;

-- Filtrar usando view para filtrar por tipo
SELECT *
FROM PROJETO_FINAL.VIEW_ALL_POKEMONS
WHERE NOME_TIPO = 'Voador';

-- Selecionar todas as vitórias filtando por treinador
SELECT *
FROM PROJETO_FINAL.View_todas_as_vitorias
where treinador_nome = 'Igor';

-- Selecionar a média de atributos por tipo de Pokémon
SELECT *
FROM PROJETO_FINAL.View_Media_Atributos_Tipo_Pokemon
WHERE NOME_TIPO = 'Fogo';

-- FUNÇÃO PARA ESCOLHER UM POKEMON POR NOME E RETORNAR TODOS OS POKEMONS DO MESMO TIPO
SELECT *
FROM encontrar_pokemon_por_tipo('machop');
